...
class BronzeAchievementProcessorImplTest {
    ...
    void eventContentShouldNotInfluenceResultIfExpHigherThan1000(HabitCompletionEvent event) {
        Optional<Badge> result = processor.process(event, 5000);
        assertTrue(result.isPresent());
        assertEquals(Badge.BRONZE, result.get());
    }
    ...
    @ParameterizedTest
    @MethodSource("provideRandomEvents")
    void eventContentShouldNotInfluenceResultIfExpLowerThan1000(HabitCompletionEvent event) {
